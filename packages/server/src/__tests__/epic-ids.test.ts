...
describe('Epic ID Generation', () => {
  ...
  it('should generate unique epic IDs even with 20+ tasks', async () => {
    ...
    expect(await storage.get('EPIC-0001')).toBeDefined();
    expect(await storage.get('EPIC-0002')).toBeDefined();
    expect(await storage.get('EPIC-0003')).toBeDefined();

    // Verify epic titles are correct (not overwritten)
    expect((await storage.get('EPIC-0001'))?.title).toBe('Epic 1');
    expect((await storage.get('EPIC-0002'))?.title).toBe('Epic 2');
    expect((await storage.get('EPIC-0003'))?.title).toBe('Epic 3');
  });
